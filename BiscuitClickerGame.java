...
public class BiscuitClickerGame {
    public static void main(String[] args) throws InterruptedException {
        /* This game was coded to be played on any screen size if played on full-screen. Many of the ratios of x, y,
        width, and height are strange because I wanted to keep the same ratios as BiscuitClickerGameMac (the original) */
        ...
        while (true) {
            ...
            String str = BiscuitClicker.titleScreen();
            BiscuitClicker.game(str);
        }
    }
}

class BiscuitClicker {
    ...
    static String titleScreen() throws InterruptedException {
        // Gets size of screen (width and height)
        ...
        // Sets bounds of the new buttons in accordance with the dimensions of the screen
        newGame.setBounds((screenSize.width * 13) / 36, ((screenSize.height * 70) / 157),
                (screenSize.width * 5) / 18, (screenSize.height * 14) / 157);
        loadGame.setBounds((screenSize.width * 13) / 36, ((screenSize.height * 90) / 157),
                (screenSize.width * 5) / 18, (screenSize.height * 14) / 157);
        quitGame.setBounds((screenSize.width * 13) / 36, ((screenSize.height * 110) / 157),
                (screenSize.width * 5) / 18, (screenSize.height * 14) / 157);
        ...
        // Declares image outside try-catch, so it can be used outside (not limited within scope of try-catch)
        Image image;

        // Gets logo png and upscales it to desired size
        try {
            BufferedImage bufferedImage = ImageIO.read(new File("src/Biscuit-Clicker-Logo.png"));
            image = bufferedImage.getScaledInstance((screenSize.width * 143) / 288, (screenSize.height * 78) / 785, Image.SCALE_DEFAULT);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        // Creates label with upscaled logo icon
        JLabel l = new JLabel(new ImageIcon(image));

        // Sets bounds of label
        l.setBounds((screenSize.width * 181) / 720, (screenSize.height * 30) / 157, (screenSize.width * 143) / 288, (screenSize.height * 78) / 785);
    ...
    static void game(String str) throws InterruptedException {
        ...
        try {
            BufferedImage bufferedImage = ImageIO.read(Objects.requireNonNull(BiscuitClickerGame.class.getResource("biscuit.png")));
            Image image = bufferedImage.getScaledInstance((screenSize.width * 3) / 140, (screenSize.width * 3) / 140, Image.SCALE_DEFAULT);
        ...
        b.setBounds((screenSize.width * 25) / 56, (screenSize.height * 137) / 314, (screenSize.width * 3) / 28, (screenSize.height * 10) / 157);
        ...
        mult.setBounds((screenSize.width - mult.getPreferredSize().width) / 2, (screenSize.height * 141) / 157, mult.getPreferredSize().width, (screenSize.height * 9) / 157);
        ...
        mult.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                if (biscuits[0] >= upgradeCost[0]) {
                    ...
                    // Changes bounds to preferred size (in case text no longer fits) and re-centers button
                    mult.setBounds((screenSize.width - mult.getPreferredSize().width) / 2, (screenSize.height * 141) / 157, mult.getPreferredSize().width, (screenSize.height * 9) / 157);
                } else
        ...
        if (biscuitsPerSec == 0)
            BPS = new JButton("<html>Increase Passive Income to 1 Biscuit/sec: <html>" + df.format(upgradeCost[1]) + "<html> Biscuits (ignoring multiplier)<html>");
        else
            BPS = new JButton("<html>Increase Passive Income to <html>" + df.format((biscuitsPerSec * 2L)) + "<html> Biscuits/sec: <html>" + df.format(upgradeCost[1]) + "<html> Biscuits (ignoring multiplier)<html>");
        ...
        if(upgradeCost[1] > 25000000L)
            BPS.setBounds((screenSize.width * 121) / 144, (screenSize.height * 71) / 157, (screenSize.width * 23) / 144, (screenSize.height * 15) / 157);
        else
            BPS.setBounds((screenSize.width * 121) / 144, (screenSize.height / 2) - BPS.getPreferredSize().height, (screenSize.width * 23) / 144, BPS.getPreferredSize().height + mult.getHeight() - mult.getPreferredSize().height);
        ...
        BPS.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                if (biscuits[0] >= upgradeCost[1]) {
                    ...
                    // Updates button text for next upgrade
                    BPS.setText("<html>Increase Passive Income to <html>" + df.format((biscuitsPerSec * 2L)) + "<html> Biscuits/sec: <html>" + df.format(upgradeCost[1]) + "<html> Biscuits (ignoring multiplier)<html>");

                    // Updates BPS text and re-centers BPS
                    // If upgrade cost is too large, also increases height of BPS so all text will fit within button
                    if(upgradeCost[1] > 25000000L)
                        BPS.setBounds((screenSize.width * 121) / 144, (screenSize.height - ((screenSize.height * 15) / 157)) / 2, (screenSize.width * 23) / 144, (screenSize.height * 15) / 157);
                    else
                        BPS.setBounds((screenSize.width * 121) / 144, (screenSize.height / 2) - BPS.getPreferredSize().height, (screenSize.width * 23) / 144, BPS.getPreferredSize().height + mult.getHeight() - mult.getPreferredSize().height);
                } else
                    // Notifies player of insufficient biscuits
        ...
        b.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                do {
                    x = (int) (Math.random() * (((screenSize.width * 25) / 28) + 1));
                    y = 25 + (int) (Math.random() * (((screenSize.height * 140) / 157) + 1));
                } while (((x > ((screenSize.width * 11) / 28) - (mult.getPreferredSize().width / 2)) && (x < (screenSize.width / 2) + (mult.getPreferredSize().width / 2)) && (y > ((screenSize.height * 131) / 157))) || ((x > (screenSize.width * 739) / 1008) && (y > ((screenSize.height * 137) / 314) - (BPS.getPreferredSize().height / 2)) && (y < (screenSize.height / 2) + mult.getHeight() - mult.getPreferredSize().height)));

                // Sets new random location of button
                b.setBounds(x, y, (screenSize.width * 3) / 28, (screenSize.height * 10) / 157);
